...
create table public.audits (id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade, project_id uuid not null references public.projects(id) on delete cascade, image_path text not null, context text, status public.audit_status not null default 'processing', overall_score smallint check (overall_score between 0 and 100), scores jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
create table public.audit_issues (id uuid primary key default gen_random_uuid(), audit_id uuid not null references public.audits(id) on delete cascade, user_id uuid not null references auth.users(id) on delete cascade, position smallint not null default 0, severity public.issue_severity not null, category text not null check (category in ('usability','accessibility','hierarchy','consistency','uxWriting')), title text not null, problem text not null, why_it_matters text not null, recommendation text not null, created_at timestamptz not null default now());
